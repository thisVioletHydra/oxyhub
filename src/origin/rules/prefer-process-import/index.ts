  ...
  create(context) {
    ...
    function isGlobalProcess(node: Identifier & { parent?: LintNode }) {
      ...
      return !variable.defs.some(
        (def) => def.type === 'ImportBinding' || def.type === 'Variable'
      );
    }

    function buildImportFix(fixer: RuleFixer) {
    ...
    return {
      ...
      Identifier(node) {
        if (hasProcessImport || reported || !isGlobalProcess(
          node
        )) {
          return;
        }
